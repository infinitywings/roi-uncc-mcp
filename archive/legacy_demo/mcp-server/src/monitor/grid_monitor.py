...
class GridMonitor:
    ...
    def _detect_anomalies(self, current_state):
        ...
        try:
            ...
            # Check voltage deviations
            for voltage_name, voltage_data in current_state.get('voltages', {}).items():
                baseline_voltage = self.baseline_state.get('voltages', {}).get(voltage_name, {})
                if baseline_voltage:
                    baseline_mag = baseline_voltage.get('magnitude', 0)
                    current_mag = voltage_data.get('magnitude', 0)
                    if baseline_mag > 0:
            ...
            # Check power deviations
            for power_name, power_data in current_state.get('powers', {}).items():
                baseline_power = self.baseline_state.get('powers', {}).get(power_name, {})
                if baseline_power:
                    baseline_mag = baseline_power.get('magnitude', 0)
                    current_mag = power_data.get('magnitude', 0)
                    if baseline_mag > 0:
                        deviation = abs(current_mag - baseline_mag) / baseline_mag
    ...
    def get_current_state(self):
        """Get current grid state with monitoring metadata"""
        try:
            # Get state from federate
            current_state = self.federate.get_current_state()
            ...
            if self.anomaly_history:
                current_state['recent_anomalies'] = list(self.anomaly_history)[-5:]  # Last 5 anomalies
    ...
    def get_vulnerability_assessment(self):
        """Get current vulnerability assessment based on monitoring data"""
        vulnerabilities = []
        
        try:
            current_state = self.federate.get_current_state()
            
            # Check for voltage vulnerabilities
            for voltage_name, voltage_data in current_state.get('voltages', {}).items():
                magnitude = voltage_data.get('magnitude', 0)
                pu_voltage = magnitude / 2401.78  # Convert to per-unit
                ...
                if pu_voltage < 0.95:
                    vulnerabilities.append({
                        ...
                        'current_value': pu_voltage,
                        'threshold': 0.95,
                        'recommendation': 'Monitor for cascading undervoltage events'
                    })
                    ...
                    vulnerabilities.append({
                        ...
                        'current_value': pu_voltage,
                        'threshold': 1.05,
                        'recommendation': 'Risk of equipment damage'
                    })
            
            # Check system health vulnerabilities
            system_health = current_state.get('system_health', {})
            if system_health.get('status') in ['degraded', 'compromised', 'critical']:
                vulnerabilities.append({
                    'type': 'system_degradation',
                    'severity': 'high' if system_health.get('status') == 'critical' else 'medium',
                    'health_score': system_health.get('score', 0),
                    'issues': system_health.get('issues', []),
                    'recommendation': 'System stressed - additional attacks likely to cause failures'
                })
            
            # Check for recent anomalies indicating vulnerabilities
            if len(self.anomaly_history) > 5:
                recent_anomalies = list(self.anomaly_history)[-10:]
                anomaly_types = {}
                for anomaly_set in recent_anomalies:
                    for anomaly in anomaly_set.get('anomalies', []):
                        anomaly_type = anomaly.get('type')
                        anomaly_types[anomaly_type] = anomaly_types.get(anomaly_type, 0) + 1
                
                for anomaly_type, count in anomaly_types.items():
                    if count > 3:  # Repeated anomalies indicate vulnerability
                        vulnerabilities.append({
                            'type': 'repeated_anomaly',
                            'anomaly_type': anomaly_type,
                            'frequency': count,
                            'severity': 'medium',
                            'recommendation': f'Investigate recurring {anomaly_type} anomalies'
                        })
    ...
    def _calculate_risk_level(self, vulnerabilities):
        ...
        if high_severity >= 3 or (high_severity >= 1 and medium_severity >= 3):
            return 'critical'
