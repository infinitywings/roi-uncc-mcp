...
class LocalLLMStrategist:
    ...
    def __init__(self, config, attack_engine, grid_monitor):
        ...
        self.llm_log_path = config.get('log_file')
        if self.llm_log_path:
            try:
                os.makedirs(os.path.dirname(self.llm_log_path), exist_ok=True)
            except Exception as exc:
                logger.warning(f"Could not prepare LLM log directory: {exc}")
    ...
    def _log_llm_interaction(self, entry):
        """Persist LLM request/response metadata to JSONL log."""
        ...
        try:
            ...
            logger.warning(f"Failed to write LLM log entry: {exc}")
    ...
    def _call_llm_api(self, messages, tools=None):
        ...
        try:
            ...
            if response.status_code != 200:
                entry["error"] = response.text[:2048]
                self._log_llm_interaction(entry)
