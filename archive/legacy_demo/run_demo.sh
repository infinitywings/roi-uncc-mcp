...
show_usage() {
    ...
    echo "                     Note: Use config/examples/ for pre-made configs"
...
if [[ -n "$MODE" && "$MODE" != "ai" && "$MODE" != "random" && "$MODE" != "comparison" ]]; then
...
if [[ "$USE_DOCKER" == "true" ]]; then
    ...
    if [[ "$USE_DOCKER" == "true" ]]; then
        ...
        if ! docker images -q roi-img:latest | grep -q .; then
            print_error "Base container 'roi-img:latest' not found"
            print_error "Please build it first: cd containers/docker && docker build -t roi-img:latest ."
            exit 1
        fi
