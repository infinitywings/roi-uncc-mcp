...
from config_converter import create_mcp_config_from_demo
...
class ContainerizedDemo:
    ...
    def __init__(self, config_file=None):
        ...
        compose_file_path = self.config.get('docker', {}).get('compose_file', 'docker/docker-compose.demo.yml')
        
        # Ensure the compose file path is relative to demo_dir
        if not os.path.isabs(compose_file_path):
            self.compose_file = os.path.join(self.demo_dir, compose_file_path)
        else:
    ...
    def check_prerequisites(self):
        ...
        result = subprocess.run(['docker', 'images', '-q', 'roi-img:latest'], 
                              capture_output=True, text=True)
        if not result.stdout.strip():
            print("❌ Base container 'roi-img:latest' not found")
            print("   Please build the base container first:")
            print("   cd containers/docker && docker build -t roi-img:latest .")
            return False
        print("✅ Base container roi-img:latest found")
    ...
    def build_mcp_container(self):
        """Build the MCP server container using docker-compose"""
        print("\n🔨 Building MCP server container...")
        
        # Get relative path to compose file from demo_dir
        compose_file_rel = os.path.relpath(self.compose_file, self.demo_dir)
        
        # Use docker-compose to build the mcp-server service
        cmd = ['docker', 'compose', '-f', compose_file_rel, 'build', 'mcp-server']
    ...
    def start_services(self):
        ...
        cleanup_cmd = ['docker', 'compose', '-f', compose_file_rel, 'down', '-v']
        ...
        cmd = ['docker', 'compose', '-f', compose_file_rel, 'up', '-d']
        process = subprocess.run(cmd, cwd=self.demo_dir)
    ...
    def _update_mcp_config(self):
        """Generate MCP configuration from demo config"""
        print("   Converting demo config to MCP server format...")
        
        # Create temporary demo config file from current config
        temp_demo_config = os.path.join(self.demo_dir, 'temp_demo_config.yaml')
        mcp_config_path = os.path.join(self.demo_dir, 'mcp-server', 'config', 'mcp.yaml')
        
        try:
            # Save current config as temporary demo config
            with open(temp_demo_config, 'w') as f:
                yaml.dump(self.config, f, default_flow_style=False, indent=2)
            
            # Convert to MCP format
            result = create_mcp_config_from_demo(temp_demo_config, mcp_config_path)
            
            if result:
                print(f"   ✅ Generated MCP config: {mcp_config_path}")
                
                # Print AI configuration being used
                if 'ai' in self.config:
                    ai_config = self.config['ai']
                    print(f"   Using AI model: {ai_config.get('model', 'Qwen/Qwen3-30B-A3B')}")
                    print(f"   Using AI base URL: {ai_config.get('api_base', 'http://nginx-lb/v1')}")
                    ...
                    if api_key:
                        ...
                        with open(api_key_file, 'w') as f:
                            f.write(api_key)
                        print(f"   ✅ API key written to {api_key_file}")
                ...
                print("   ❌ Failed to convert demo config to MCP format")
            
            # Clean up temporary file
            if os.path.exists(temp_demo_config):
                os.remove(temp_demo_config)
    ...
    def cleanup(self):
        ...
        cmd = ['docker', 'compose', '-f', compose_file_rel, 'down', '-v']
        subprocess.run(cmd, cwd=self.demo_dir)
